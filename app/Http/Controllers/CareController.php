...
class CareController extends Controller
{
    ...
    {
        // バリデーションの項目名を JS に合わせる
        $request->validate([
            'client_id'   => 'required',
            'recorded_at' => 'required', // date から recorded_at に修正
            'content'     => 'required',
        ], [
            'recorded_at.required' => '日付は必ず指定してください。',
        ]);

        try {
            // 新規作成または更新
            $record = $request->edit_record_id 
                ? CareRecord::findOrFail($request->edit_record_id) 
                : new CareRecord();

            $record->client_id           = $request->client_id;
            $record->schedule_id         = $request->schedule_id; // スケジュール紐付け
            $record->recorded_at         = $request->recorded_at; // JSから届く "2026-01-11 16:00" をそのまま入れる
